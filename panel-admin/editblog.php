...
if(isset($_POST['sub'])){
    ...
    $add_blog = $conn->prepare('UPDATE blog SET title=? , poster=? , writer=? , vio=? , date=? , content=? , caption_box=? WHERE id=?');
...
<div>
    <h1 style="margin-right: 280px; margin-top: 12px; opacity: 0.7;">ویرایش مقاله</h1>
</div>
<div class="form-menus" style="width: 70%; margin-right: 280px; margin-top: 20px;">
    <form method="POST" enctype="multipart/form-data">
        ...
        <textarea name="content" class="editor" id="my-editor" cols="30" rows="10"
            placeholder="مقاله خودتون رو بنویسید"><?= $blogs['content'] ?></textarea><br>


        <label>کپشن باکس</label>
        <input type="text" name="caption_box" class="form-control" placeholder="کپشن باکس(20 الی 30 کلمه)" value="<?= $blogs['caption_box'] ?>"><br>

        <label>نویسنده</label>
        <select name="writer" class="form-control">
            <?php foreach($admin as $teache){ ?>
            <option value="<?= $teache['id'] ?>" <?php if($teache['id'] == $blogs['writer']){ ?> selected <?php } ?>><?= $teache['username'] ?></option>
            <?php } ?>
        </select><br>


        <div style="display: flex; justify-content: center;">
            <input type="submit" value="ویرایش مقاله" name="sub" class="btn btn-warning" style="width: 60%;">
        </div>
    </form><br>
</div>
